    ...
    componentDidMount() {
        setInterval(() => {
            if(!this.state.playing){
                axios.get('https://donation-system1.herokuapp.com:' + process.env.PORT + '/donations/queue')
                .then(response => {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if(prevState.donations != this.state.donations){
            if(this.state.donations[0] != null){
                ...
                axios.post('https://donation-system1.herokuapp.com:' + process.env.PORT + '/donations/update/' + donation.id, donation)
